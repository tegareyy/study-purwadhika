...
console.log(`diameter = ${2*radius}, circumference = ${(2*Math.PI*radius).toFixed(4)}, area = ${area1}`);
...
const diffMiliDetik: number = Math.abs(new Date(date2).getTime() - new Date(date1).getTime());
